...
#include "simglb.h"
#include "config.h"
//ashwinm #include "../../frontpanel/frontpanel.h"
#include "memory.h"
#include "lcd_emu.h"
...
static void save_core(void)
{
	...
	write(fd, (char	*) mem_base(), MEMORY_SIZE);
	close(fd);
}
...
int load_core(void)
{
	...
	read(fd, (char *) mem_base(), MEMORY_SIZE);
	close(fd);
...
static int load_mos(int fd, char *fn)
{
	...
	count = mem_base() + MEMORY_SIZE - 1 - wrk_ram;
	if ((readn = read(fd, (char *) wrk_ram, count)) == count) {
		printf("Too much to load, stopped at 0x%04x", MEMORY_SIZE-1);
		rc = 1;
	}
	close(fd);
